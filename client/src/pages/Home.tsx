      ...
      <div className="relative flex min-h-screen">
        ...
        <main className="min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-[1480px] space-y-8 p-5 md:p-10">
            ...
            {section === "clients" && <DataSection title="Clientes" detail="Saúde do relacionamento, dados de contato e histórico." count={clients.data?.length ?? 0}>{clients.data?.length ? clients.data.map(client => <div key={client.id} className="flex items-center justify-between rounded-xl border border-white/[.06] bg-white/[.02] p-4"><div><div className="font-medium">{client.name}</div><div className="text-sm text-slate-500">{client.email}{client.company ? ` · ${client.company}` : ""}</div></div><div className="text-right"><div className="text-sm text-emerald-300">{client.healthScore}/100</div><div className="text-xs text-slate-600">índice de saúde</div></div></div>) : <EmptyState icon={Users} title="Ainda não há clientes cadastrados" detail="O CRM está pronto para receber sua primeira ficha." action="Adicionar cliente" onClick={() => toast.info("Formulário de CRM na próxima iteração.")} />}</DataSection>}
            {section === "projects" && <DataSection title="Projetos" detail="Status, entregas, progresso e revisões validadas no servidor." count={projects.data?.length ?? 0}>{projects.data?.length ? projects.data.map(project => <div key={project.id} className="rounded-xl border border-white/[.06] bg-white/[.02] p-4"><div className="flex justify-between"><div className="font-medium">{project.title}</div><Badge className="border-emerald-400/20 bg-emerald-400/10 text-emerald-300">{statusLabel[project.status]}</Badge></div><div className="mt-3 flex items-center gap-3"><Progress value={project.progress} className="h-1.5 bg-white/10" /><span className="text-xs text-slate-500">{project.progress}%</span></div></div>) : <EmptyState icon={BriefcaseBusiness} title="Ainda não há projetos" detail="Crie o primeiro projeto e controle o ciclo completo." action="Criar projeto" onClick={() => toast.info("Formulário de projeto na próxima iteração.")} />}</DataSection>}
